...
function attempt_login(string $username,string $password): ?string {
    ...
    if (defined('SUPERADMIN_USER')&&defined('SUPERADMIN_PASS')&&$username===SUPERADMIN_USER&&hash_equals(SUPERADMIN_PASS,$password))
        return issue_token(build_payload(0,$username,'superadmin',null,null));
    try{
    ...
    return issue_token(build_payload((int)$row['id'],$row['username'],$row['role'],$row['tenant_id']!==null?(int)$row['tenant_id']:null,$row['tenant_name']));
}
function build_payload(int $uid,string $username,string $role,?int $tenantId,?string $tenantName): array {
...
function issue_token(array $payload): string {
    $token=jwt_encode($payload,JWT_SECRET);$ttl=defined('JWT_TTL')?JWT_TTL:28800;
    $https=(!empty($_SERVER['HTTPS'])&&$_SERVER['HTTPS']!=='off')||(int)($_SERVER['SERVER_PORT']??80)===443;
    setcookie('jwt',$token,['expires'=>time()+$ttl,'path'=>'/','secure'=>$https,'httponly'=>true,'samesite'=>'Lax']);
    return $token;
}
function logout(): void {
    $https=(!empty($_SERVER['HTTPS'])&&$_SERVER['HTTPS']!=='off')||(int)($_SERVER['SERVER_PORT']??80)===443;
    setcookie('jwt','',['expires'=>time()-3600,'path'=>'/','secure'=>$https,'httponly'=>true,'samesite'=>'Lax']);
}
